...
class Spider:
    ...
    def fetchData(self, url, parameters=None, retry=0):
        ...
        if self.opener is None:
            self.opener = self.createOpener(myheaders, self.createCookieJarHandler())
            urllib2.install_opener(self.opener)
        try:
    ...
    def downloadFile(self, url, downloadPath, proxyHandler=None, notifier=None, retry=0, max_retry=1):
        try:
            ...
            if os.path.exists(downloadPath) and os.path.getsize(downloadPath) == totalSize:
                if notifier is not None:
            ...
            if retry < max_retry:
                if notifier is not None:
                    notifier.emit('<font color=black><b>Will retry after 1 seconds.</b></font>')
                time.sleep(1)
                if notifier is not None:
                    notifier.emit('<font color=black><b>Retry...</b></font>')
                self.downloadFile(url, downloadPath, proxyHandler, notifier, retry + 1, max_retry)
            else:
                if notifier is not None:
